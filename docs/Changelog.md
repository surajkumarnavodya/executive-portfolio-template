...
complete narrative (research findings, design tradeoffs, and every
verification step). Summary:

### Fixed (Security — live personal-data exposure)
- **`studio.html` was live and publicly reachable at the real domain**
  (GitHub Pages serves the repo root as-is). Its "Export Website ZIP"
  feature fetches same-origin sibling files (`index.html`,
  `assets/js/config.js`, `portfolio.json`) and zips them — anyone visiting
  the live `studio.html` could download the real name, contact details and
  résumé path. New `.github/workflows/deploy-live.yml` + `tools/
  package-live.ps1`/`.sh` publish a live-only artifact (the Studio/template
  system excluded entirely) as the GitHub Pages deployment instead of Pages
  serving the repo root — requires a one-time manual switch in the GitHub UI
  (Settings → Pages → Source → "GitHub Actions").
- **The Portfolio Copilot's knowledge base had real personal data hardcoded
  directly in `assets/js/ui.js`** (employer, metrics, certifications, email,
  phone, LinkedIn) — invisible to any HTML-only content audit, and shipped
  in the shared JS bundle regardless of which product loaded it.
  `cfg.copilot` (kb/fallback/greet) now overrides `ui.js`'s `DEFAULT_KB` when
  present; `config.demo.js` supplies a genericized version for the template.
- **`portfolio.json`** contained real identity data mixed with a fictional
  testimonial in the same file. Fixed to correct, real data only; the
  fictional testimonial removed.
- **Studio's `buildExportManifest`** hardcoded real file paths
  (`index.html`, `assets/js/config.js`, `portfolio.json`,
  `assets/images/profile.jpg`). Now returns `{src, dest}` pairs so a buyer's
  exported site fetches template-safe equivalents while still landing at
  the expected file names — defense-in-depth independent of deployment
  topology.

### Fixed (Architecture — component-registration defect)
- `assets/js/components.js` registered every component via unconditional
  `document.querySelector()` against hardcoded selectors (`#top`, `#about`,
  `#impact`, tag `footer`) at bundle-load time — any page sharing the bundle
  that happened to reuse one of those ids was silently opted in.
  `engineering.html` carried a 20-line comment explaining why it avoided
  `id="top"`/`id="main"` as a workaround. Switched every registration to
  `[data-component="name"]` — index.html already carried these attributes
  (added in v1.2.0, never wired up). `engineering.html` now uses ordinary
  `id="top"`/`id="main"` again; the workaround comment is gone.
- Found and fixed a second, dormant instance of the same defect:
  `engineering.html` independently had `id="contact"` and a bare `<footer>`,
  both matching the old hardcoded selectors — silently registering (never
  rendered into, since nothing called those renderers without `config.js`
  loaded, but not safe).
- Found and removed a real, unrelated bug during the same audit: the
  `timeline` component's registered selector (`#experience-content`) didn't
  match where its `data-component="timeline"` attribute actually lived
  (`#leadership`, a different section) — a stale leftover. Nothing calls
  `render('timeline', ...)` anywhere, so deleted the dead registration
  rather than fix-and-keep a landmine.
- `customizer.js`'s Visual Layout Builder (`applySectionLayout()`) bypassed
  `components.js` entirely — raw `document.getElementById('main')`. Gated
  on a new explicit `data-layout-root` marker instead.

### Added (Template product)
- `index.template.html`, `portfolio.template.json`,
  `assets/images/profile-placeholder.{jpg,webp,avif}` — genericized content
  fork with bracketed placeholders (`[Your Name]`, `[Amount]`, etc.) over
  every real name/metric/employer/certification/award/testimonial found —
  regenerated by `tools/build_template_content.js`, which exits non-zero on
  any content drift rather than silently under-genericizing.
- `tools/package-live.ps1`/`.sh` and `tools/package-template.ps1`/`.sh` —
  two packaging profiles via `git archive` against a real commit, each
  excluding exactly what the other product must never contain.
- `tools/build_bundle_js.js` — closes a gap this repo has carried since
  v1.0: the JS bundle was always hand-concatenated (the direct cause of the
  v1.5.0 regression below). Supports `--override` for template-safe file
  substitution during packaging.
- `tools/build_template_ui.js` — genericizes `assets/js/ui.js`'s Copilot KB
  for the template package.

### Verified
End-to-end with Playwright against a local server at every step: component
registration (11 components on `index.html`, zero on `engineering.html`,
Visual Layout Builder reordering still works), the Copilot on all three
surfaces (real KB on `index.html`/`engineering.html`, placeholder KB on
`index.template.html`), and a full build-and-extract of both packages with
an exhaustive case-insensitive residual scan for real-data patterns across
every file — clean except LICENSE.txt's required, intentional copyright
notice.
