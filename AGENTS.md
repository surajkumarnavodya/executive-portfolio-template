...
> Ported from `.github/copilot-instructions.md`. Both files are kept in sync — when you update one, mirror the change into the other so Copilot and Codex share the same memory.
...
**Sync gap note:** this file fell behind `.github/copilot-instructions.md`/`CLAUDE.md` by three sessions — a credibility/accuracy corrections pass (Executive Scorecard removed, "8+ years"→"7+ years", SEO/a11y cleanup), the new `engineering.html` technical-audience page, and a site-wide font-size switcher (A/A+/A++ dropdown). Not backfilled here (see `CLAUDE.md` for full detail on all three); picking back up below with the session after those.
...
**Note:** this file was several sessions behind `CLAUDE.md`/`.github/copilot-instructions.md` as of the entry below (missing the credibility-corrections pass, the `engineering.html` session, and the font-size-switcher session) — not backfilled, flagged for whoever does that next.
...
### v1.6.0 — Live/template architecture split
User asked for a migration plan (via `EnterPlanMode`) to split the live portfolio from the commercial template, citing two content systems that never connected and `engineering.html`'s 20-line defensive comment working around `components.js`'s unconditional-registration defect. Research found something more urgent: GitHub Pages serves the repo root as-is, `studio.html` was confirmed live at the real domain, and its Export ZIP feature fetches same-origin `index.html`/`config.js`/`portfolio.json` and zips them — a live data-exfiltration path. Sequenced as 7 commits, each Playwright-verified: (1) checkpoint ~30 files of prior uncommitted work; (2) `tools/package-live.ps1`/`.sh` + `.github/workflows/deploy-live.yml` to stop Pages serving the repo root (user still needs to flip Settings → Pages → Source to "GitHub Actions"); (3) `components.js` registration switched from hardcoded id/tag selectors to `[data-component="name"]` — index.html already carried these attributes since v1.2.0, never wired up; found a second dormant instance of the same defect (`engineering.html`'s `#contact`/`<footer>` also matched the old selectors); (4) deleted the dead `timeline` component (its attribute was on the wrong section, a stale bug), gated the Visual Layout Builder on a new `data-layout-root` marker, simplified `engineering.html` back to ordinary `id="top"`/`id="main"`; (5) `tools/build_template_content.js` regenerates a genericized `index.template.html` (~103 find/replace entries, exits non-zero on any mismatch), fixed `portfolio.json` (dropped a fictional testimonial sitting next to real identity data), made `portfolio-data-service.js`'s export manifest fetch template-safe file substitutes; (6) `tools/package-template.ps1`/`.sh`; (7) found while verifying step 6's actual output — `assets/js/ui.js`'s Portfolio Copilot had this deployment's real employer/metrics/certifications/contact info hardcoded in its knowledge base, compiled into the shared JS bundle regardless of product. Fixed with a `cfg.copilot` override (real `DEFAULT_KB` stays, since `engineering.html` needs it with no config.js) plus two new build tools (`build_template_ui.js`, `build_bundle_js.js` — the latter finally closes a JS-bundle-hand-maintenance gap this repo carried since v1.0, the direct cause of the v1.5.0 regression). Final verification: built and grepped the actual template zip for real-data patterns — clean except three low-stakes, judged-acceptable matches (LICENSE.txt's required notice, one demo org name, changelog history). Full detail in `CLAUDE.md` and `docs/Changelog.md` v1.6.0.
